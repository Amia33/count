...
function getCountImage(params) {
  ...
  if (!(theme in themeList)) theme = "asoul";
  ...
  const defs = uniqueChar.reduce((ret, cur) => {
    ...
    ret = `${ret}
    <image id="${cur}" width="${toFixed(width, 5)}" height="${toFixed(
      height,
      5
    )}" xlink:href="${data}" />`;
...
<svg viewBox="0 0 ${toFixed(x, 5)} ${toFixed(y, 5)}" width="${toFixed(
    x,
    5
  )}" height="${toFixed(
    y,
